...
float3 LightPos <string uiname="Light Position Distance";> = 0;
    ...
    float4 TexCd : TEXCOORD0)
{
	...
	// PHONG STUFF
	float3 LightDirW = normalize(lPos - Out.wPos);
...
outcolor PS_shadows(vs2ps In): COLOR
{
	...
	col.c1.rgb *= PhongPoint(In.wPos, In.NormV, In.ViewDirV, In.LightDirV);
